...
fn main() -> ! {
    ...
    let mut _matrix0 = LedMatrix::new(clock0, data0, strobe0, HEIGHT, WIDTH);
    let mut _matrix1 = LedMatrix::new(clock1, data1, strobe1, HEIGHT, WIDTH);
    ...
    loop {
        if usb_serial.poll() {
            let mut buf = [0u8; 4];
            if usb_serial.read(&mut buf).is_ok() {
                hprintln!("Rx: {:?}", &buf);
            }
        }
    }
